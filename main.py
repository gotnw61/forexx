...
def main():
    ...
    # Doğrudan Streamlit'i çalıştır (run_streamlit_app fonksiyonu yerine)
    import subprocess
    print("Streamlit uygulaması başlatılıyor...")
    streamlit_process = subprocess.Popen(["streamlit", "run", os.path.join(ROOT_DIR, "ui/streamlit_app.py")])
    
    try:
        # Bot'u başlat
        bot.start()
        
        # Streamlit işlemi tamamlanana kadar bekle

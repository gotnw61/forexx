...
class BrokerConnector:
    ...
    def connect(self) -> bool:
        ...
        if mt5 is None:
            logger.error("MetaTrader5 modülü yüklü değil")
            return False
            
        try:
            # MT5 ayarlarını al
            mt5_path = self.settings.get("mt5", {}).get("path", "")
            ...
            if not mt5.terminal_info():
                logger.error("MT5 terminal bilgisi alınamadı")
                return False
                
            # Hesaba giriş yap
            login = self.settings.get_api_key("mt5.login")
            password = self.settings.get_api_key("mt5.password")
            server = self.settings.get_api_key("mt5.server")
            ...
            if account_info:
                logger.info(f"Hesap Bakiyesi: {account_info.balance} {account_info.currency}")
                logger.info(f"Hesap Tipi: {'Gerçek' if account_info.trade_mode == 0 else 'Demo'}")
            
            return True
        except Exception as e:
            logger.error(f"MT5 bağlantı hatası: {e}")
            self.connected = False
            return False
